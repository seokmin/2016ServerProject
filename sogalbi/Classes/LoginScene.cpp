...
// 	return;
	...
	else
	{
		...
		auto address = "http://13.94.44.159:8258/Request/Login";
...
void LoginScene::logoutButtonClicked(Ref* pSender)
{
	...
	auto request = new network::HttpRequest();
	auto address = "http://13.94.44.159:8258/Request/Logout";
	request->setUrl(address);
	request->setRequestType(network::HttpRequest::Type::POST);
...
void LoginScene::rechargeButtonCLicked(Ref * pSender)
{
	...
	auto request = new network::HttpRequest();
	auto address = "http://13.94.44.159:8258/Request/Recharge";
	request->setUrl(address);
	request->setRequestType(network::HttpRequest::Type::POST);
